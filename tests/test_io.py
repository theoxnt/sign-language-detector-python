...
@pytest.mark.parametrize(
    ...
    [
        ("prompt", int, "string", 5, TypeError, "min must be the same type as cast_type"),
        ("prompt", int, 5, "string", TypeError, "max must be the same type as cast_type")
    ]
)
def test_ask_ko_type_with_all_parameters(prompt, cast_type, min, max, expected_errors, match_msg):
    with pytest.raises(expected_errors, match=match_msg):
